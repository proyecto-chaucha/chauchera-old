...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Chaucha: starting difficulty is 1 / 2^12
...
{
    // Chaucha: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 30 * 60; // Chaucha: 30 minutos
static const int64 nTargetSpacing = 1 * 60; // Chaucha: 1 minuto
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if  (DiffMode == 1) { 
        return GetNextWorkRequired_Norm(pindexLast, pblock); 
    }
    else if (DiffMode == 2) { 
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chaucha: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        // The original date was August 11. But Heellxz was drunk or something
        // And wrote September 11. Odd date for Chilean People and USA too.
        // PD: Pico pal que lee 8=D.
        ...
        assert(block.hashMerkleRoot == uint256("0x1b54ad13e84ece043533beb59d6b666047ffc77a4496034a101791601d711998"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xaa, 0xa2, 0x26, 0xa9 }; // Chaucha: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChauchaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChauchaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChauchaMiner(CWallet *pwallet)
{
    printf("ChauchaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chaucha-miner");
    ...
    try { loop {
        ...
        printf("Running ChauchaMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChauchaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChauchaMiner, pwallet));
}
